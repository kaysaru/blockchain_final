...
export default function useNFTMint() {
  ...
  const { getProviderOrSigner, connectWallet, walletConnected } = useWeb3();
  ...
  const renderButton = () => {
    ...
    if (loading) {
      return (
        <LoadingButton
          ...
          variant="contained"
          size="large"
          endIcon={<CircularProgress color="secondary" />}
        >
          Loading...
        </LoadingButton>
      );
    ...
    if (!presaleStarted) {
      return <Typography>Presale hasnt started!</Typography>;
    }
